...
export const mergeAndDistinct = <T, K>(arr1: T[], arr2: T[], property: keyof T): T[] => {
  const mergedArray: T[] = [...arr1, ...arr2];

  const mergedSet = new Set<K>();
  const mergedResult: T[] = [];

  for (const item of mergedArray) {
    const itemKey = item[property] as K;

    if (!mergedSet.has(itemKey)) {
      mergedSet.add(itemKey);
